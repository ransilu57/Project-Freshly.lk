...
import { Search, Filter, Eye, X, RefreshCw, Sprout } from 'lucide-react';
...
const ProductListing = () => {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 py-12 font-sans bg-white min-h-screen">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4">
        ...
        <button
          ...
          className="flex items-center px-5 py-2.5 bg-green-600 text-white rounded-full hover:bg-green-700 transition-colors shadow-md"
      ...
      {/* Filters */}
      <div className="sticky top-0 z-10 bg-white rounded-xl shadow-lg p-6 mb-8 -mx-6">
        <div className="flex flex-row justify-center gap-4 max-w-4xl mx-auto">
          <div className="relative flex-grow">
            <input
              ...
              className="w-full px-5 py-3 pl-12 border border-gray-200 rounded-full shadow-sm focus:outline-none focus:ring-2 focus:ring-green-400 bg-gray-50 transition-all duration-300 text-gray-800 placeholder-gray-400"
          ...
          <div className="relative w-56">
            <select
              ...
              className="w-full px-5 py-3 pl-4 pr-12 border border-gray-200 rounded-full shadow-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-400 appearance-none transition-all duration-300 text-gray-800"
            ...
            <Filter className="absolute right-4 top-1/2 transform -translate-y-1/2 text-green-500" size={22} />
          </div>
        </div>
      </div>
      ...
      {loading && (
        <div className="product-grid grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[...Array(8)].map((_, index) => (
            <div
              key={index}
              className="border border-gray-200 rounded-2xl shadow-sm p-5 bg-white animate-pulse flex flex-col min-w-[150px]"
            >
      ...
      {/* Error State */}
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-800 px-6 py-4 rounded-xl shadow-sm flex flex-col sm:flex-row items-center justify-between mb-8">
          <span className="text-lg">{error}</span>
          <button
            onClick={fetchProducts}
            className="mt-4 sm:mt-0 flex items-center px-4 py-2 bg-red-600 text-white rounded-full hover:bg-red-700 transition-colors shadow-md"
      ...
      <div className="product-grid grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filteredProducts.length ? (
          filteredProducts.map((product) => (
            <div
              key={product._id}
              className="border border-gray-200 rounded-2xl shadow-md p-6 bg-gradient-to-br from-white to-green-50 transition-all duration-300 hover:shadow-xl hover:-translate-y-2 flex flex-col relative overflow-hidden min-w-[150px]"
            >
              {/* Certification Badge */}
              {product.certification && (
                <span
                  className={`absolute top-4 right-4 z-10 text-xs font-semibold px-3 py-1 rounded-full shadow-sm ${
              ...
              <h3 className="text-xl font-bold text-green-900 mb-2 line-clamp-1">
              ...
              <button
                ...
                className="mt-auto w-full px-5 py-3 bg-green-600 text-white rounded-full hover:bg-green-700 transition-colors flex items-center justify-center shadow-md"
          ...
          !loading && (
            <div className="col-span-full text-center py-16 bg-white rounded-xl shadow-md">
              ...
              <p className="text-gray-600 mt-2 mb-6">Try adjusting your search or explore other categories.</p>
              <button
                onClick={() => setCategory('')}
